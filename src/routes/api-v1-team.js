...
function createApiV1TeamRouter(supabaseClient, twilioClient) {
  ...
  // POST /api/v1/team/invites — creates a pending invite and texts the
  // accept link. Doctor/owner only (both currently arrive as role "doctor" —
  ...
  router.post("/invites", requireRole("doctor", "owner"), async (req, res) => {
    ...
    try {
      ...
      if (twilioClient && config.APP_BASE_URL) {
        ...
        try {
          await twilioClient.sendWhatsApp({ to: phone, from: clinic?.whatsappFrom, body });
        } catch (err) {
          req.log?.warn({ err }, "[api-v1:team] invite send failed — invite still created");
        }
      }

      return res.status(201).json({ success: true, data: { invite }, message: null });
    } catch (err) {
